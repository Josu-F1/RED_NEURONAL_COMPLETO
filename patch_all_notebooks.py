...
for path in notebooks:
    try:
        ...
        # Append the new cells if not already appended
        # Quick check if ROC is already there
        has_roc = any('roc_curve' in ''.join(c['source']) for c in nb['cells'] if c['cell_type'] == 'code')
        if not has_roc:
        ...
        if modified:
            ...
            print(f"Already patched or not modified: {path}")
