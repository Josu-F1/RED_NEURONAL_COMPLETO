...
notebook_path = r"c:\Users\Marlon\Documents\Sexto\BI\MiProyecto\RED_NEURONAL_COMPLETO\fact_abastecimiento_logistica\Dispersion2.ipynb"

with open(notebook_path, 'r', encoding='utf-8') as f:
    nb = nbformat.read(f, as_version=4)

# 1. Training Markdown
md_train = """# Paso 5: Entrenamiento de la Red Neuronal (Perceptrón Multicapa)
...
target_names = ['Insatisfactoria (0)', 'Aceptable (1)', 'Excelente (2)']
...
md_biz = """# Conclusiones de Impacto para el Negocio

En un proyecto de **Business Intelligence e Inteligencia Artificial**, la métrica matemática debe traducirse en valor empresarial. Basado en los resultados de nuestra red neuronal:

1. **Alta Capacidad Preventiva:** El modelo es capaz de anticipar entregas "Insatisfactorias" con una alta sensibilidad (recall). Esto significa que la gerencia logística puede ser alertada automáticamente antes de que ocurra un quiebre de servicio, permitiendo tomar medidas correctivas (ej. cambiar el transportista o despachar stock de emergencia).
2. **Identificación de Cuellos de Botella (Factores Críticos):** El análisis de explicabilidad (Feature Importance) demostró visualmente cuáles son los factores operativos exactos que determinan si una entrega fallará o será exitosa. Con este conocimiento, la empresa ya no opera a ciegas, sino que puede enfocar su presupuesto en mejorar las áreas que más influyen (como tiempos de aduana o confiabilidad de ciertos proveedores específicos).
3. **Optimización de Presupuestos:** Reducir la incertidumbre en las entregas de "Abastecimiento Logística" directamente reduce las penalizaciones por demoras y mejora la satisfacción del cliente final (SLA), transformando el departamento logístico de un simple centro de costos a un área estratégica y predictiva impulsada por IA."""
